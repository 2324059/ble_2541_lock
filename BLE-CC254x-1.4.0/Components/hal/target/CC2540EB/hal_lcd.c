...
#define LCD_RST P1_7       //_RES     复位 
#define LCD_DC  P1_2       //A0    H：数据，L:命令
...
/*********************LCD写数据************************************/ 
void HalLcd_HW_Write_Dat(uint8 dat)     
{
    uint8 i=0;
...
/*********************LCD写命令************************************/                                        
void HalLcd_HW_Write_Cmd(uint8 cmd)
{
    uint8 i = 0;
...
{ 
    HalLcd_HW_Write_Cmd(0xb0+y);
    HalLcd_HW_Write_Cmd(((x&0xf0)>>4)|0x10);
    HalLcd_HW_Write_Cmd((x&0x0f)|0x01); 
} 
/*********************LCD全屏************************************/
void LCD_Fill(unsigned char bmp_dat) 
{
    ...
    {
        HalLcd_HW_Write_Cmd(0xb0+y);
        HalLcd_HW_Write_Cmd(0x01);
        HalLcd_HW_Write_Cmd(0x10);
        for(x=0;x<X_WIDTH;x++)
            HalLcd_HW_Write_Dat(bmp_dat);
    }
}
...
{  
    ...
    LCD_SCL=1;
    LCD_RST=0;
    HalLcd_HW_WaitUs(10000);
    LCD_RST=1;      //从上电到下面开始初始化要有足够的时间，即等待RC复位完毕   
    HalLcd_HW_Write_Cmd(0xae);//--turn off oled panel
    HalLcd_HW_Write_Cmd(0x00);//---set low column address
    HalLcd_HW_Write_Cmd(0x10);//---set high column address
    HalLcd_HW_Write_Cmd(0x40);//--set start line address  Set Mapping RAM Display Start Line (0x00~0x3F)
    HalLcd_HW_Write_Cmd(0x81);//--set contrast control register
    HalLcd_HW_Write_Cmd(0xcf); // Set SEG Output Current Brightness
    HalLcd_HW_Write_Cmd(0xa1);//--Set SEG/Column Mapping     0xa0左右反置 0xa1正常
    HalLcd_HW_Write_Cmd(0xc8);//Set COM/Row Scan Direction   0xc0上下反置 0xc8正常
    HalLcd_HW_Write_Cmd(0xa6);//--set normal display
    HalLcd_HW_Write_Cmd(0xa8);//--set multiplex ratio(1 to 64)
    HalLcd_HW_Write_Cmd(0x3f);//--1/64 duty
    HalLcd_HW_Write_Cmd(0xd3);//-set display offset    Shift Mapping RAM Counter (0x00~0x3F)
    HalLcd_HW_Write_Cmd(0x00);//-not offset
    HalLcd_HW_Write_Cmd(0xd5);//--set display clock divide ratio/oscillator frequency
    HalLcd_HW_Write_Cmd(0x80);//--set divide ratio, Set Clock as 100 Frames/Sec
    HalLcd_HW_Write_Cmd(0xd9);//--set pre-charge period
    HalLcd_HW_Write_Cmd(0xf1);//Set Pre-Charge as 15 Clocks & Discharge as 1 Clock
    HalLcd_HW_Write_Cmd(0xda);//--set com pins hardware configuration
    HalLcd_HW_Write_Cmd(0x12);
    HalLcd_HW_Write_Cmd(0xdb);//--set vcomh
    HalLcd_HW_Write_Cmd(0x40);//Set VCOM Deselect Level
    HalLcd_HW_Write_Cmd(0x20);//-Set Page Addressing Mode (0x00/0x01/0x02)
    HalLcd_HW_Write_Cmd(0x02);//
    HalLcd_HW_Write_Cmd(0x8d);//--set Charge Pump enable/disable
    HalLcd_HW_Write_Cmd(0x14);//--set(0x10) disable
    HalLcd_HW_Write_Cmd(0xa4);// Disable Entire Display On (0xa4/0xa5)
    HalLcd_HW_Write_Cmd(0xa6);// Disable Inverse Display On (0xa6/a7) 
    HalLcd_HW_Write_Cmd(0xaf);//--turn on oled panel
    LCD_Fill(0x00); //初始成黑屏
    LCD_Set_Pos(0,0);   
